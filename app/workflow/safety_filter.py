...
class SafetyFilter:
    ...
    def validate(self, text: str, sender: str = "", context: dict[str, Any] | None = None) -> SafetyCheckResult:
        """
        ...
        if not any(text_lower.count(phrase) > 0 for phrase in ["thank", "appreciate", "looking forward"]):
            if self.strict_mode:
    ...
    def _generate_suggestions(
        ...
        if any("scheduling" in v.lower() for v in violations):
            suggestions.append("Use phrases like 'Let's discuss scheduling' instead of confirming dates")
