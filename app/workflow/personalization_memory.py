...
class PersonalizationMemory:
    ...
    def get_tone_effectiveness(self) -> dict[str, float]:
        ...
        for tone, stats in tone_stats.items():
            if stats["total"] > 0:
                effectiveness[tone] = (
                    stats["approved"] + 0.5 * stats["edited"]
                ) / stats["total"]
            else:
    ...
    def get_personalization_hints(self) -> dict[str, Any]:
        ...
        approved_edits = [
            a for a in actions
            if a.get("action") in ["approved", "edited"]
        ]
        ...
        lengths = [self._infer_length(a.get("user_action_text", a.get("generated_text", ""))) for a in approved_edits]
